...
$(document).ready(function () {
  $("#frmAlisFaturasiEkle").on("submit",function (e) {
    e.preventDefault();
    var izin = true;

    var formVerileri = $("form#frmAlisFaturasiEkle").serializeJSON();
    formVerileri["txtAlisFaturasiCariIdsi"] = $("#txtAlisFaturasiCariIdsi").attr("data-id");

    console.log(formVerileri["txtAlisFaturasiCariIdsi"]);
    if (!formVerileri["txtAlisFaturasiCariIdsi"]) {
      $.notify({
        // options
    ...
    if (izin) {
      ...
      jQuery.ajax({
        type: "POST",
        url: ""+yolHtml+"fatura/alis-faturasi-ekle",
        data: {alisFaturasiEkle:json},
        cache: false,
        beforeSend: function() {
        ...
        success: function(response)
        {
          ...
          if (donut.yanit["yanit"] == true) {
            ...
            $("form#frmAlisFaturasiEkle input").val("");
          }else {
            $.notify({
              // options
              icon: 'fa fa-danger fa-2x',
