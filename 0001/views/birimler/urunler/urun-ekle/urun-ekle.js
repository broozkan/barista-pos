$(document).ready(function () {
  ...
  $(document).on("click",".btnAltUrunStokUrunBilgileriEkle",function () {
  ...
  $(document).on("click",".txtAltUrunStokTakibiYapilsinMi",function () {
    var val = $(this).val();
    if (val == 1) {
      $(this).closest(".divAltUrunEkle").find(".divAltUrunStokTakibiBilgileri").removeClass("d-none");
    }else {
      $(this).closest(".divAltUrunEkle").find(".divAltUrunStokTakibiBilgileri").addClass("d-none");
    }
  })
  ...
  var click = 0;
  $(document).on("click",".btnYeniAltUrunEkle",function () {
    click++;
    var vergilerHtml = $("#txtUrunAlisVergiIdsi").html();

    var eklenecekHtml = '<div class="panel-body divAltUrunEkle">';
    ...
    eklenecekHtml += '<button type="button" class="btn btn-sm btn-default buttonInsideInput btnVergiEkle" data-toggle="modal" data-target="#modalYeniVergiEkle" name="button">Vergi Ekle</button>';
    eklenecekHtml += '</div>';
    eklenecekHtml += '</div>';
    eklenecekHtml += '<div class="col-md-6">';
    ...
    eklenecekHtml += '<div class="input-group">';
    eklenecekHtml += '<label for="email_address">Ekle</label>';
    eklenecekHtml += '<button type="button" class="btn-sm btn g-bg-cgreen btnAltUrunStokUrunBilgileriEkle"><span class="zmdi zmdi-plus"></span> </button>';
    eklenecekHtml += '</div>';
    eklenecekHtml += '</div>';
    eklenecekHtml += '<div class="col-md-6">';
    ...
    $(".divAltUrunEkle:last").after(eklenecekHtml);
  })

  $(document).on("click",".btnAltUrunSil",function () {
    var altUrunSayisi = $(".divAltUrunEkle").length;
    if (altUrunSayisi < 2) {

    }else {
      $(".divAltUrunEkle:last").remove();
    }

  })

  $("form#frmUrunEkle").on("submit",function (e) {
    e.preventDefault();
    ...
    var formVerileri = $("form#frmUrunEkle").serializeJSON();
    ...
    formData.append("urunEkle",json);

    var altUrunEklenecekMi = $("#collapseAltUrunEkle").hasClass("show");

    var altUrunBilgileri;

    if ( altUrunEklenecekMi ) {
      altUrunBilgileri = new Array();
      altUrunStokTakibiBilgileri = new Array();
      var altUrunIndexi = 0;
      $(".divAltUrunEkle").each(function () {
        ...
        if (altUrunStokTakibiYapilacakMi == 1) {
          ...
          $(this).find(".tblAltUrunStokTakibiBilgileri tbody tr").each(function () {
            var dusulecekStokUrunIdsi = $(this).attr("id");
            var dusulecekStokMiktari = $(this).attr("data-id");
            var dusulecekStokJson = {"txtDusulecekStokUrunIdsi":dusulecekStokUrunIdsi,"txtDusulecekStokMiktari":dusulecekStokMiktari,"txtAltUrunIndexi":altUrunIndexi};
      ...
      formData.append("altUrunEkle",altUrunBilgileri);

    }else {
      formData.append("altUrunEkle",false);
    }


    jQuery.ajax({
      type: "POST",
      url: ""+yolHtml+"urun/urunEkle/",
      xhr: function() {
      ...
      success: function(response)
      {
        ...
        if (donut.yanit["yanit"] == true) {
          ...
          $("form#frmUrunEkle input").val("");
          $("#collapseAltUrunEkle").removeClass("show");
        }else {
          $.notify({
            // options
            icon: 'fa fa-danger fa-2x',
